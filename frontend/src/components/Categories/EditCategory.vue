                ...
                <div class="form-group row" :class="{ 'has-danger': errors.has('title') }">
                    ...
                    <div class="col-10">
                        <input id="title" name="title" type="text" class="form-control" data-vv-rules="required"
                            data-vv-as="Category title" v-model="category.title" v-validate>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        async saveCategory() {
            // const record = await this.$pb.collection('categories').update(this.id, this.category);

            await this.$pb.collection('categories').update(this.id, this.category)
                .then((response) => {
