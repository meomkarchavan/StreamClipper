<template>
    <div>
        <div>
            ...
            <div>
                <div>
                    <input type="text" v-model="videoUrl" placeholder="Enter YouTube video URL" class="form-control">
                    <br>
                    <button @click="getVideoDetails" class="btn btn-primary">Get Video Details</button>
                    <br>
                </div>
                <div></div>
            </div>
...
export default {
    ...
    methods: {
        ...
        async saveVideo() {
            ...
            await this.$pb.collection('videos').create(this.video)
                .then((response) => {
                    this.video = response;
        ...
        getVideoDetails() {
            ...
            this.$player.addEventListener('onStateChange', (event) => {
                if (event.data === window.YT.PlayerState.PLAYING) {
                    ...
                    const video = {
                        youtube_title: this.$player.getVideoData().title,
                        duration: duration,
                        url: this.videoUrl
                    }
